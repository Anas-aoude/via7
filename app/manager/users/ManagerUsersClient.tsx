...
const manageableRoles: UserRole[] = ["USER", "HOST", "VIP_HOST", "AGENCY"];
      ...
      <div className="bg-white rounded-2xl w-full max-w-xl p-6">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div>
            ...
            <input
              disabled
              value={selectedUser.name || "No name"}
              className="mt-2 w-full border rounded-lg p-3 bg-neutral-100"
            />
          </div>
...
const ManagerUsersClient: React.FC<ManagerUsersClientProps> = ({ users }) => {
  ...
  const openEditModal = (user: ManagerUser) => {
  ...
  const onSave = async () => {
    ...
    axios
      .patch(`/api/manager/users/${selectedUser.id}`, {
        role,
        isBanned,
      })
      .then(() => {
            ...
            {paginatedUsers.map((user) => (
              <tr key={user.id} className="border-t hover:bg-neutral-50">
                ...
                <td className="p-4">
                  <button
                    onClick={() =>
                      router.push(`/manager/users/${user.id}/listings`)
                    }
...
export default ManagerUsersClient;