...
const invalidateListingCaches = async (
  ...
  if (failedResults.length > 0) {
    console.error(
      "ADMIN_LISTING_CACHE_INVALIDATION_ERROR",
      failedResults
    );
  }
};
...
export async function PATCH(
  ...
  try {
    ...
    const rateLimit = await RateLimitService.check({
      key: `admin-listing-update:${currentUser.id}`,
      limit: 100,
      windowSeconds: 60 * 60,
    });
  ...
  try {
    ...
    const rateLimit =
      await RateLimitService.check({
        key: `admin-listing-delete:${currentUser.id}`,
        limit: 30,
        windowSeconds: 60 * 60,
      });
