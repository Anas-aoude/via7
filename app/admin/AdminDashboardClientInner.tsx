...
function getCumulativeData(users: UserItem[], listings: ListingItem[]) {
  const map = new Map<
    string,
    {
      date: string;
...
const AdminDashboardClient: React.FC<AdminDashboardClientProps> = ({
  ...
  const usersGrowthData = useMemo(() => {
    return getCumulativeData(filterByRange(users, usersGrowthRange), []);
  }, [users, usersGrowthRange]);

  const listingsGrowthData = useMemo(() => {
    return getCumulativeData([], filterByRange(listings, listingsGrowthRange));
  }, [listings, listingsGrowthRange]);

  const governorateData = useMemo(
    () =>
      groupByCount(
        filterByRange(listings, governorateRange),
        (listing) => listing.governorate,
        12
      ),
    [listings, governorateRange]
  );

  const categoryData = useMemo(
    () =>
      groupByCount(
        filterByRange(listings, categoryRange),
        (listing) => listing.category,
        10
      ),
    [listings, categoryRange]
  );

  const purposeData = useMemo(
    () =>
      groupByCount(
        filterByRange(listings, purposeRange),
        (listing) => listing.purpose || "Unknown"
      ),
    [listings, purposeRange]
  );

  const statusData = useMemo(() => {
    const filtered = filterByRange(listings, statusRange);
  ...
  }, [listings, statusRange]);

  const featuredData = useMemo(() => {
    const filtered = filterByRange(listings, featuredRange);
  ...
  }, [listings, featuredRange]);

  const topHostsData = useMemo(() => {
    const filtered = filterByRange(listings, hostsRange);
  ...
  const roleData = useMemo(() => {
    return groupByCount(users, (user) => user.role);
  }, [users]);
  ...
  const cards = [
    ...
    },
    {
      ...
      href: "/admin/listings?active=true",
    },
    {
      label: "Audit Log",
      value: usersCount,
